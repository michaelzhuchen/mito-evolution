...
iqtree2 -s $MSA_FILE -m MF+MERGE -cmin 4 -cmax 12 -p $PARTITIONFILE -nt $N_CORES -safe -seed 42 -rcluster 10 --prefix $OUTPUT_PREFIX

## 1b. Run with partition model with pre-existing model selection. Specify constraint tree version
...
iqtree2 -s $MSA_FILE -p $PARTITIONFILE -g $CONSTRAINT_TREE --prefix $OUTPUT_PREFIX -nt $N_CORES -bb 1000 -alrt 1000 -safe -seed 42 -bnni
