...
# Localization label weight vector: based on the frequency of each label in the training data. Precomputed weights available below for full and leave-one-out training datasets.
...
def focal_loss(input, target, gamma=1):
    ...
    focal_loss = (focal_loss * mask).sum() / mask.sum()
    return focal_loss
