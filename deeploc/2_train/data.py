...
def get_swissprot_df(clip_len):  
    ...
    data_df = pd.read_csv(LOCALIZATION_DATA, dtype={"species": "string"})
...
class DataloaderHandler:
    ...
    def get_train_val_dataloaders(self, outer_i):
        ...
        train_dataloader = torch.utils.data.DataLoader(train_dataset, collate_fn=TrainBatchConverter(self.alphabet, self.embed_len), batch_sampler=train_batches, num_workers=4, pin_memory=True)
        ...
        val_dataloader = torch.utils.data.DataLoader(val_dataset, collate_fn=TrainBatchConverter(self.alphabet, self.embed_len), batch_sampler=val_batches, num_workers=4, pin_memory=True)
