...
def get_best_threshold_mcc(y_true, y_prob):
    ...
    y_true_obs = y_true[mask].astype(int)
    y_pred_obs = y_pred[mask].astype(int)

    score = matthews_corrcoef(y_true_obs, y_pred_obs)
...
def get_optimal_threshold_mcc(output_df, data_df):
    ...
    for i in range(11):
        ...
        mask = ~np.isnan(actuals[:, i])
        optimal_thresholds[i] = get_best_threshold_mcc(actuals[mask, i], predictions[mask, i])
...
def calculate_sl_metrics_fold(test_df, thresholds):
    ...
    mask = ~np.isnan(y_membrane)
    ypred_membrane = ypred_membrane[mask].astype(int)
    y_membrane = y_membrane[mask].astype(int)
    # mask = ~np.isnan(y_subloc)
    # ypred_subloc = ypred_subloc[mask].astype(int)
    # y_subloc = y_subloc[mask].astype(int)
    ...
    metrics_dict["NumLabels"] = np.nanmean(np.nansum(y_subloc, axis=1))

    # metrics_dict["NumLabelsTest"] = ypred_subloc.sum(1).mean()
    metrics_dict["NumLabelsTest"] = np.nanmean(np.nansum(ypred_subloc, axis=1))
    ...
    mask = ~np.isnan(y_subloc)
    element_match = (~mask) | (ypred_subloc == y_subloc)
    row_match = np.all(element_match, axis=1)
    any_nonNA = mask.any(axis=1)
    metrics_dict["ACC_subloc"] = row_match[any_nonNA].mean()
    
    y_subloc_flat = y_subloc.ravel()
    ypred_subloc_flat = ypred_subloc.ravel()
    mask = ~np.isnan(y_subloc_flat)
    y_subloc_flat_obs = y_subloc_flat[mask].astype(int)
    ypred_subloc_flat_obs = ypred_subloc_flat[mask].astype(int)

    # metrics_dict["HammLoss_subloc"] = 1-hamming_loss(y_subloc_flat_obs, ypred_subloc_flat_obs)
    #metrics_dict["Jaccard_subloc"] = jaccard_score(y_subloc, ypred_subloc, average="samples")
    #metrics_dict["MicroF1_subloc"] = f1_score(y_subloc_flat_obs, ypred_subloc_flat_obs, average="micro")
    ...
    for i in range(10):
        ...
        mask = ~np.isnan(y_subloc[:,i])
        metrics_dict[f"{CATEGORIES[1+i]}"] = matthews_corrcoef(y_subloc[mask,i], ypred_subloc[mask,i])
