...
# Map the merged OGs to raw OGs
merged_OG_mapping <- read.table(here("data/orthogroups/first_merge", "initially_merged_OG_id_mapping.txt"), sep="\t")
colnames(merged_OG_mapping) <- c("merged_OG_id", "raw_OG_id")
merged_OG_seprows$OG_id[merged_OG_seprows$OG_id %in% merged_OG_mapping$merged_OG_id] <- merged_OG_mapping$raw_OG_id[match(merged_OG_seprows$OG_id[merged_OG_seprows$OG_id %in% merged_OG_mapping$merged_OG_id], merged_OG_mapping$merged_OG_id)]
merged_OG_seprows <- merged_OG_seprows %>% separate_rows(OG_id, sep=", ")
...
## Write out
write.table(orthogroups_merge, here("data/orthogroups/second_merge", "second_merged_orthogroups.tsv"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
